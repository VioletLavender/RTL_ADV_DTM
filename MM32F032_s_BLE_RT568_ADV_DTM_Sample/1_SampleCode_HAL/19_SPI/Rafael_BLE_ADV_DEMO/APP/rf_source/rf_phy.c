...
void initBLEsw(void)
{
    ...
    ble_device_param.ble_deviceAddr_param.addr[0] = 0x11;
    ble_device_param.ble_deviceAddr_param.addr[1] = 0x22;
    ble_device_param.ble_deviceAddr_param.addr[2] = 0x33;
    ble_device_param.ble_deviceAddr_param.addr[3] = 0x44;
    ble_device_param.ble_deviceAddr_param.addr[4] = 0x55;
    ble_device_param.ble_deviceAddr_param.addr[5] = 0x76;
...
{
    SPI_PDMA_SetTx(RFIP_REG_112, (uint32_t)(initParaAddr), LEN_CRC_INIT );
}
...
{
    addr_TxFIFO = 2;
    SPI_PDMA_SetTx(TX_BUFFER_WRITE_PORT, (uint32_t)(AdvAddr), SIZE_BLE_ADDR  );
...
{
    addr_TxFIFO = 8;
    SPI_PDMA_SetTx(TX_BUFFER_WRITE_PORT, (uint32_t)(SrcAddr), LEN_ADV_SCAN_DATA_MAX  );
...
{
    SPI_PDMA_SetTx(RFIP_REG_139, (uint32_t)addrAesKey, LEN_AES_KEY );
}


void smpAES_in_key(Uint8 *key)
{
    SPI_PDMA_SetTx(RFIP_REG_139, (uint32_t)key, LEN_AES_KEY  );
}
...
{
    addr_TxFIFO = 0;
    SPI_PDMA_SetTx(TX_BUFFER_WRITE_PORT, (uint32_t)plaintextData, LEN_AES_KEY  );
}
...
void smpAES_out(Uint8 *encDataOut)
{
    ...
    SPI_PDMA_SetRx_Isr(RX_BUFFER_READ_PORT, (uint32_t)encDataOut, LEN_AES_KEY );
...
void smpAES_outInv(Uint8 *encDataOut)
{
    ...
    while ((SPI_1BYT_SetRx_Isr(RFIP_REG_155)&REG_155_AES_BUSY));
    SPI_PDMA_SetRx_Isr(RX_BUFFER_READ_PORT, (uint32_t)i, LEN_AES_KEY );
    SPI_1BYT_SetTx_Isr(RFIP_REG_155, REG_155_AES_MODE_BYPASS);
